...
describe 'Titles Api', type: :request do
  it 'returns all titles' do
    get 'https://localhost:3000/titles'
...
describe 'POST /titles' do
  ...
  let(:url){'https://localhost:3000/titles'}
  let(:params){{title: {show_id: '123', title:'teste', show_type: 'teste', release_year:'1999', country:'brasil', listed_in:'2000', description:'teste'}}}
...
describe 'DELETE /titles/:id' do
  ...
  it 'delete a title' do
    expect{delete("https://localhost:3000/titles/#{title.id}").to change(Title, :count).by(0)}
  end
end

describe 'UPDATE /titles/:id' do
  let (:title){create(:title)}
  let(:url){"https://localhost:3000/titles/#{title.id}"}
  let(:params){{title: {show_id: '123', title:'teste', show_type: 'teste', release_year:'1999', country:'brasil', listed_in:'2000', description:'teste'}}}
